...
class RunPredict(object):
    def __init__(self, request_paras, station_infos, prefix_predicted, folder, use_caiyun, auto_submit=True):
    ...
    def predict(self):
        ...
        data_folder = request_data(self.request_paras, now, use_caiyun=self.use_caiyun)#抓取数据
        ...
        predict_file = os.path.join(self.folder, self.prefix_predicted+ now.strftime("%Y-%m-%d-%H") +'.csv')
    ...
    def predict_data(self,data_folder,file_name=None):
        ...
        #data_folder = request_data(self.request_paras, now)#抓取数据
        ...
        if file_name == None:
            ...
            predict_file = os.path.join(self.folder, self.prefix_predicted+ file_name +'.csv')
        pd_predict = pd.DataFrame.from_dict(data_predict)
        pd_predict.to_csv(predict_file, columns=['test_id','PM2.5','PM10','O3'], index=False)
    ...
    def test_standard(self, predict_file, predict_time):
        '''
        测试预报的精度, 一站点同一时刻某参数缺失则整条放弃
        predict_time:进行预报的时刻
        '''
